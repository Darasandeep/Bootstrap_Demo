...
QUnit.test('Testing calculatetotal function with several sets of inputs', function (assert) {
    assert.equal(calculatetotal(1, 150), 300, 'Tested with two relatively small positive numbers');
    
    assert.equal(calculatetotal(500, 500), 250000, 'Tested with two large positive numbers. Any arguments greater than 100 will be set to 100.');
        //throws( block                                    [, expected ] [, message ] ) 
    assert.throws(function () { calculatetotal(null); }, /The given argument is not a number/, 'Passing in null correctly raises an Error');
        //throws( block                                    [, expected ] [, message ] ) 
